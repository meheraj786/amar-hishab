  ...
  return (
    <div className="min-h-screen bg-[#F4F2EE] p-4 md:p-8 lg:p-12 pb-28">
      <header className="flex justify-between items-start md:items-center mb-10">
        ...
        <div className="flex flex-col md:flex-row justify-end gap-2 md:gap-3 w-full md:w-auto">
          <button
            onClick={onAddClick}
            className="bg-primary text-white px-6 py-4 rounded-2xl font-black shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
            <ReceiptText size={20} strokeWidth={3} /> নতুন এন্ট্রি
          </button>
          <Link
            to="/invoice"
            className="text-primary border-2 border-primary px-6 py-4 rounded-2xl font-black shadow-sm hover:bg-primary/5 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-8">
          <div className="relative overflow-hidden bg-[#F0F5FF] rounded-[2.5rem] p-8 text-white shadow-2xl border border-white/5">
            ...
            <div className="relative z-10 space-y-6">
              <p className="text-sm font-black text-primary/80 uppercase tracking-widest">
                মোট ক্যাশ ব্যালেন্স
              </p>
              <h2 className="text-5xl font-black text-primary tracking-tighter">
                ৳ {getBalance().toLocaleString("bn-BD")}
              </h2>
              <div className="pt-6 border-t border-primary/20 flex justify-between items-center">
                <div>
                  <p className="text-[12px] font-black text-primary/60 uppercase">
                    সর্বমোট লাভ
                  </p>
          ...
          <div className="space-y-3">
            {recentTransactions.length === 0 ? (
              ...
              recentTransactions.map((tx) => (
                <div
                  key={tx.id}
                  className="bg-white p-5 rounded-[2rem] border border-transparent hover:border-primary/10 shadow-sm flex items-center justify-between group transition-all"
                >
                      ...
                      <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                        ...
                        {tx.type === "income" && tx.profit! > 0 && (
                          <span className="text-emerald-500 ml-1">
                            • লাভ ৳{tx?.profit?.toLocaleString("bn-BD")}
                          </span>
                        )}
                      </p>
                    </div>
