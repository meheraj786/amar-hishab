...
import { Search, Phone, User, Banknote, Trash2, Calendar } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#F4F2EE] p-4 md:p-8 lg:p-12 pb-32">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-black text-slate-900">বাকির <span className="text-primary">খাতা</span></h1>
        ...
        <div className="grid gap-4">
            ...
            <Card key={due.id} className="border-none shadow-sm rounded-[2rem] bg-white overflow-hidden hover:shadow-md transition-all">
              <CardContent className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400"><User size={28}/></div>
                  <div>
                    <h3 className="font-black text-xl text-slate-900">{due.customerName}</h3>
                ...
                <div className="flex items-center justify-between md:justify-end gap-8 border-t md:border-t-0 pt-4 md:pt-0">
                  <div className="text-left md:text-right">
                    ...
                    <p className="text-[9px] font-bold text-slate-300">মোট: ৳{due.totalAmount}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => confirm("মুছে ফেলবেন?") && deleteDue(due.id)} className="p-3 text-slate-200 hover:text-rose-500 transition-colors bg-slate-50 rounded-xl"><Trash2 size={20}/></button>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {filteredDues.length === 0 && (
            <div className="bg-white rounded-[3rem] p-20 text-center border-2 border-dashed border-slate-100">
               <p className="text-slate-300 font-black text-lg italic">কোনো বাকি পাওয়া যায়নি</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Dialog open={!!payTarget} onOpenChange={() => setPayTarget(null)}>
        <DialogContent className="rounded-[2.5rem] border-none p-8 bg-white max-w-sm shadow-2xl">
          ...
          <div className="space-y-5 pt-4">
            <div className="text-center p-4 bg-slate-50 rounded-2xl">
               <p className="text-xs font-bold text-slate-400 uppercase">কাস্টমার: {payTarget?.customerName}</p>
               <p className="text-lg font-black text-slate-700">বাকি আছে: ৳{payTarget?.dueAmount}</p>
            </div>
            ...
            <Button onClick={() => { payDue(payTarget.id, payAmount); setPayTarget(null); }} className="w-full h-16 rounded-2xl bg-emerald-600 hover:bg-emerald-700 font-black text-lg shadow-xl shadow-emerald-100 transition-all active:scale-95">পরিশোধ নিশ্চিত করুন</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
