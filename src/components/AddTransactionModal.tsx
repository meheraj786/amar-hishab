...
import { CheckCircle2, ShoppingBag, Banknote, Layers, User, Plus, Trash2, Phone, Ruler, TrendingUp } from "lucide-react";
...
export default function AddTransactionModal({ open, onOpenChange, editingTransaction = null }: any) {
      ...
      watchedTransactions.forEach((item: any, index: number) => {
        if (item.type === "income" && Number(item.yardsSold) > 0) {
          if (Number(item.pricePerYard) > 0) {
            const amt = Math.round(Number(item.yardsSold) * Number(item.pricePerYard));
            if (item.amount !== amt) setValue(`transactions.${index}.amount`, amt);
          }
          if (Number(item.costPricePerYard) > 0) {
            const cost = Math.round(Number(item.yardsSold) * Number(item.costPricePerYard));
            if (item.costPrice !== cost) setValue(`transactions.${index}.costPrice`, cost);
          }
        }
      });
    }
  ...
  const bulkTotalProfit = watchedTransactions?.reduce((acc: number, curr: any) => {
    if (curr.type === "income") {
  ...
  const onSubmit = async (data: any) => {
    if (mode === "single") {
      if (data.isDue && data.type === 'income') addDue({ ...data, customerName: data.customerName || "অজানা", customerPhone: data.customerPhone || "", totalAmount: data.amount, dueAmount: data.amount });
      else editingTransaction ? updateTransaction(editingTransaction.id, data) : addTransaction(data);
    } else {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {watch("type") === 'income' && (
                <div className="grid grid-cols-3 gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="space-y-1"><Label className="text-[10px] font-black uppercase text-slate-400">বেচা (গজ)</Label><Input type="number" step="any" {...register("pricePerYard")} placeholder="0" className="h-12 bg-white border-none rounded-xl font-bold" /></div>
                  <div className="space-y-1"><Label className="text-[10px] font-black uppercase text-slate-400">কেনা (গজ)</Label><Input type="number" step="any" {...register("costPricePerYard")} placeholder="0" className="h-12 bg-white border-none rounded-xl font-bold" /></div>
                  <div className="space-y-1"><Label className="text-[10px] font-black uppercase text-slate-400">মোট গজ</Label><Input type="number" step="any" {...register("yardsSold")} placeholder="0" className="h-12 bg-white border-none rounded-xl font-bold" /></div>
                </div>
              )}

              <div className={watch("type") === 'income' ? "grid grid-cols-2 gap-3" : "w-full"}>
                <div className="space-y-1"><Label className="text-[10px] font-black text-slate-400">{watch("type") === 'income' ? "বেচা দাম" : "খরচের পরিমাণ"}</Label><div className="relative"><Banknote className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={18}/><Input type="number" {...register("amount")} className="pl-10 h-14 text-2xl font-black bg-slate-100 border-none rounded-2xl" /></div></div>
                {watch("type") === 'income' && <div className="space-y-1"><Label className="text-[10px] font-black text-slate-400">কেনা দাম</Label><div className="relative"><ShoppingBag className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={18}/><Input type="number" {...register("costPrice")} className="pl-10 h-14 text-2xl font-black bg-slate-100 border-none rounded-2xl" /></div></div>}
              </div>

              {sType === "income" && (sTotalAmount > 0 || sTotalCost > 0) && (
                <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  <div className="flex items-center gap-2 text-emerald-600 font-black"><TrendingUp size={18}/><span>সম্ভাব্য লাভ</span></div>
                  <span className="text-xl font-black text-emerald-700">৳{(sTotalAmount - sTotalCost).toLocaleString()}</span>
                </div>
              )}

              <div className="grid grid-cols-2 gap-3">
                <select {...register("paymentMethod")} className="h-12 px-4 font-bold bg-slate-50 rounded-xl border-none outline-none"><option value="cash">নগদ</option><option value="mobile">মোবাইল</option><option value="bank">ব্যাংক</option></select>
                <select {...register("category")} className="h-12 px-4 font-bold bg-slate-50 rounded-xl border-none outline-none">{popularCategories[watch("type") as "income" | "expense"].map(c => <option key={c} value={c}>{c}</option>)}</select>
              </div>

              {watch("type") === 'income' && (
                <div className="flex items-center gap-3 p-3 bg-primary/5 rounded-2xl border border-primary/10">
                  <input type="checkbox" {...register("isDue")} id="due-check" className="w-5 h-5 accent-primary rounded" />
                  <Label htmlFor="due-check" className="font-bold text-primary">এটি কি বাকি?</Label>
                  {watch("isDue") && <div className="flex-1 flex gap-2 animate-in slide-in-from-left-1"><Input {...register("customerName")} placeholder="নাম" className="h-9 bg-white border-none rounded-lg text-xs" /><Input {...register("customerPhone")} placeholder="ফোন" className="h-9 bg-white border-none rounded-lg text-xs" /></div>}
                </div>
              )}
                  ...
                  <div key={field.id} className="p-4 bg-slate-50 rounded-[2rem] border border-slate-100 space-y-4 relative">
                    <div className="flex justify-between items-center px-1">
                       <select {...register(`transactions.${index}.type`)} className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg border-none outline-none ${rowType === "income" ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}>
                          <option value="income">বিক্রয়</option><option value="expense">খরচ</option>
                       </select>
                       <button type="button" onClick={() => remove(index)} className="text-rose-400"><Trash2 size={14}/></button>
                    </div>
                    ...
                    <div className={rowType === 'income' ? "grid grid-cols-2 gap-2" : "w-full"}>
                       <Input type="number" {...register(`transactions.${index}.amount`)} placeholder={rowType === 'income' ? "টাকা" : "খরচের পরিমাণ"} className="h-10 text-sm font-black rounded-lg border-none bg-white shadow-sm w-full" />
                       {rowType === 'income' && <Input type="number" {...register(`transactions.${index}.costPrice`)} placeholder="খরচ" className="h-10 text-sm font-bold rounded-lg border-none bg-white shadow-sm" />}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                       <select {...register(`transactions.${index}.paymentMethod`)} className="h-10 px-2 font-bold bg-white rounded-lg border-none shadow-sm text-xs">
                          <option value="cash">নগদ</option><option value="mobile">মোবাইল</option><option value="bank">ব্যাংক</option>
                       </select>
                    ...
                    {rowType === 'income' && (
                      <div className="flex flex-col gap-2 p-2 bg-white rounded-lg shadow-sm border border-primary/5">
                        <div className="flex items-center gap-2"><input type="checkbox" {...register(`transactions.${index}.isDue`)} className="w-4 h-4 accent-primary rounded" /><span className="text-[10px] font-black text-primary uppercase tracking-tighter">বাকি বিক্রয়?</span></div>
                        {rowIsDue && <div className="grid grid-cols-2 gap-2"><Input {...register(`transactions.${index}.customerName`)} placeholder="নাম" className="h-8 text-[10px] border-none bg-slate-50" /><Input {...register(`transactions.${index}.customerPhone`)} placeholder="ফোন" className="h-8 text-[10px] border-none bg-slate-50" /></div>}
                      </div>
                    )}
                    <div className="flex justify-between items-center"><Input type="date" {...register(`transactions.${index}.date`)} className="h-8 text-[10px] border-none bg-transparent w-28" /><Input {...register(`transactions.${index}.notes`)} placeholder="নোট..." className="h-8 text-[10px] border-none bg-transparent flex-1 text-right italic" /></div>
                  </div>
                );
              })}
              <Button type="button" variant="outline" onClick={() => append({ type: "income", date: getLocalDate(), amount: "", isDue: false, paymentMethod: "cash", category: "পণ্য বিক্রয়" })} className="w-full border-dashed border-2 rounded-2xl h-14 text-slate-400 font-bold hover:text-primary transition-all flex items-center justify-center"><Plus size={16} className="mr-2" /> আরও হিসাব যোগ করুন</Button>
            </div>
          )}
          ...
          {mode === "bulk" && bulkTotalProfit > 0 && (
             <div className="p-4 bg-emerald-600 rounded-2xl text-white flex justify-between items-center shadow-lg shadow-emerald-100">
                <span className="font-black text-xs uppercase tracking-widest">সব মিলিয়ে মোট লাভ</span>
                <span className="text-2xl font-black">৳{bulkTotalProfit.toLocaleString()}</span>
             </div>
          )}
